...
|-------------------------|-------------------------------------------------------|--------|
| Intel RealSense Camera Mount      | Intel RealSense Camera mounted to car and ready to use in race    |    10    |

#### Software
| Quest Name | Description | Score |
|---|---|---|
| SDF Based Collision Detection Node | Build a node that generates SDFs from occupancy grids and detects collisions to assist the local planner | 10 |
| Hybrid A* + Path Optimization Node | Develop and integrate a Hybrid A* path planner with a path optimizer, validated in sim and on the car | 10 |
| Intel RealSense Software Integration | Install ROS drivers and integrate the Intel RealSense camera to produce depth, point cloud, and image data | 10 |
| Race-Ready Tuning | Tune the full stack for competition-level performance, targeting up to 10 m/s top speed | 3 |
| Infrastructure Improvements | Set up Foxglove with documentation and build an automated rosbag recording script on launch | 2 |